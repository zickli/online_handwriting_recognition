...
from source.dataset import BaseDataset
...
class HandWritingDatasetConditional(HandWritingDataset):
    ...
    def __init__(self, data_path, var_len_seq=None, use_bow_labels=True):
        ...
        self.boc_labels = self.data_dict['boc_labels'] if 'boc_labels' in self.data_dict.keys() else self.data_dict[
            'soc_labels']
        self.eow_labels = self.data_dict['eow_labels']
        self.bow_labels = self.data_dict['bow_labels'] if 'bow_labels' in self.data_dict.keys() else self.data_dict[
            'sow_labels']

        # sequence length, strokes, targets (i.e., strokes+end-of-character).
        # The dimensions with None will be padded if seq_len isn't passed.
        self.sample_shape = [[], [self.sequence_length, sum(self.input_dims)],
                             [self.sequence_length, sum(self.target_dims)]]
    ...
    def sample_generator(self):
        ...
        Returns:
            (generator): each sample is a list of data elements.
        """
        for stroke, char_label, eoc_label, bow_label in zip(self.samples, self.one_hot_char_labels, self.eoc_labels,
                                                            self.bow_labels):
            bow_label_ = np.expand_dims(bow_label, axis=1)
            eoc_label_ = np.expand_dims(eoc_label, axis=1)
            if self.use_bow_labels:
                yield [stroke.shape[0], np.float32(np.hstack([stroke, char_label, bow_label_])),
                       np.float32(np.hstack([stroke, char_label, eoc_label_, bow_label_]))]
            else:
                yield [stroke.shape[0], np.float32(np.hstack([stroke, char_label])),
                       np.float32(np.hstack([stroke, char_label, eoc_label_]))]
    ...
    def fetch_sample(self, sample_idx):
        ...
        if self.use_bow_labels:
            ...
            return [stroke.shape[0], np.expand_dims(np.float32(np.hstack([stroke, char_label, bow_label])), axis=0),
                    np.expand_dims(np.float32(np.hstack([stroke, char_label, eoc_label, bow_label])), axis=0)]
        else:
            return [stroke.shape[0], np.expand_dims(np.float32(np.hstack([stroke, char_label])), axis=0),
                    np.expand_dims(np.float32(np.hstack([stroke, char_label, eoc_label])), axis=0)]
...
class HandWritingClassificationDataset(HandWritingDatasetConditional):
    ...
    def sample_generator(self):
        ...
        if self.bow_target:
            for stroke, char_label, eoc_label, bow_label in zip(self.samples, self.one_hot_char_labels, self.eoc_labels,
                ...
                yield [stroke.shape[0], stroke_augmented, np.float32(
                    np.hstack([char_label, np.expand_dims(eoc_label, -1), np.expand_dims(bow_label, -1)]))]
        else:
            for stroke, char_label, eoc_label in zip(self.samples, self.one_hot_char_labels, self.eoc_labels):
                if self.data_augmentation:
                    ...
                    stroke_augmented = stroke
                yield [stroke.shape[0], stroke_augmented,
                       np.float32(np.hstack([char_label, np.expand_dims(eoc_label, -1)]))]
    ...
    def fetch_sample(self, sample_idx):
        ...
        if self.bow_target:
            return [stroke.shape[0], stroke,
                    np.expand_dims(np.float32(np.hstack([char_label, eoc_label, bow_label])), axis=1)]
        else:
...
def main():
    ...
    data_path = "./data/deepwriting_validation.npz"
    dataset = Dataset(data_path)

    sample_idx = 1

    stroke_sample = dataset.samples[sample_idx]
    char_label = dataset.one_hot_char_labels[sample_idx]
    eoc_label = dataset.eoc_labels[sample_idx]
    bow_label = dataset.bow_labels[sample_idx]

    print(stroke_sample.shape)
    print(char_label.shape)
    print(eoc_label.shape)
    print(bow_label.shape)

    print("label:", dataset.texts[sample_idx])

    chars_idx = np.where(eoc_label == 1)
    chars_val = dataset.one_hot_to_int_labels(char_label[chars_idx])
    chars = dataset.char_encoder.inverse_transform(chars_val)

    print("reconstruct:", ''.join(chars))

    stroke = dataset.undo_normalization(stroke_sample)
